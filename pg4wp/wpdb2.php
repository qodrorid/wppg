...
class wppg_db {
	...
	public function __construct( $dbuser, $dbpassword, $dbname, $dbhost ) {
		...
		if ( function_exists( 'mysqli_connect' ) ) {
			...
			} elseif ( version_compare( phpversion(), '5.5', '>=' ) || ! function_exists( 'wppg_sql_connect' ) ) {
	 ...
	 * @since 3.1.0
	 *
	 * @param resource $dbh     The resource given by wppg_sql_connect
	...
	public function set_charset( $dbh, $charset = null, $collate = null ) {
		...
		if ( $this->has_cap( 'collation' ) && ! empty( $charset ) ) {
			...
			if ( $this->use_mysqli ) {
				...
				if ( function_exists( 'wppg_sql_set_charset' ) && $this->has_cap( 'set_charset' ) ) {
					$set_charset_succeeded = wppg_sql_set_charset( $charset, $dbh );
				}
				if ( $set_charset_succeeded ) {
					$query = $this->prepare( 'SET NAMES %s', $charset );
					if ( ! empty( $collate ) )
						$query .= $this->prepare( ' COLLATE %s', $collate );
					wppg_sql_query( $query, $dbh );
				}
			}
		}
	}
	...
	public function select( $db, $dbh = null ) {
		...
		if ( $this->use_mysqli ) {
			...
			$success = wppg_sql_select_db( $db, $dbh );
		}
		if ( ! $success ) {
			$this->ready = false;
	...
	/**
	 * Real escape, using mysqli_real_escape_string() or wppg_sql_real_escape_string()
	 *
	 * @see mysqli_real_escape_string()
	 * @see wppg_sql_real_escape_string()
	 * @since 2.8.0
	 * @access private
	 *
	...
	function _real_escape( $string ) {
		if ( $this->dbh ) {
			if ( $this->use_mysqli ) {
				...
				return wppg_sql_real_escape_string( $string, $this->dbh );
			}
		}
	...
	public function flush() {
		...
		if ( $this->use_mysqli && $this->result instanceof mysqli_result ) {
			...
			wppg_sql_free_result( $this->result );
		}
	}
	...
	public function db_connect( $allow_bail = true ) {
		...
		if ( $this->use_mysqli ) {
			...
			// like wppg_sql_connect does. This duplicates how wppg_sql_connect detects a port and/or socket file.
			$port = null;
			$socket = null;
			$host = $this->dbhost;
			...
			if ( $this->dbh->connect_errno ) {
				...
				if ( $this->has_connected ) {
					...
					$attempt_fallback = false;
				} elseif ( ! function_exists( 'wppg_sql_connect' ) ) {
					$attempt_fallback = false;
				}
			...
			if ( WP_DEBUG ) {
				$this->dbh = wppg_sql_connect( $this->dbhost, $this->dbuser, $this->dbpassword, $new_link, $client_flags );
			} else {
				$this->dbh = @wppg_sql_connect( $this->dbhost, $this->dbuser, $this->dbpassword, $new_link, $client_flags );
			}
		}
	...
	public function check_connection( $allow_bail = true ) {
		if ( $this->use_mysqli ) {
			...
			if ( ! empty( $this->dbh ) && wppg_sql_ping( $this->dbh ) ) {
				return true;
			}
		}
	...
	public function query( $query ) {
		...
		if ( ! empty( $this->dbh ) ) {
			if ( $this->use_mysqli ) {
				if ( $this->dbh instanceof mysqli ) {
					$wppg_sql_errno = mysqli_errno( $this->dbh );
				} else {
				...
				if ( !empty( $this->dbh ) ) {
					$wppg_sql_errno = wppg_sql_errno( $this->dbh );
				} else {
					$wppg_sql_errno = 2006;
				}
			}
		...
		if ( empty( $this->dbh ) || 2006 == $wppg_sql_errno ) {
		...
		if ( $this->use_mysqli ) {
			...
			if ( !empty( $this->dbh ) ) {
				$this->last_error = wppg_sql_error( $this->dbh );
			} else {
				$this->last_error = __( 'Unable to retrieve the error message from MySQL' );
			}
		}
		...
		if ( preg_match( '/^\s*(create|alter|truncate|drop)\s/i', $query ) ) {
			...
			if ( $this->use_mysqli ) {
				...
				$this->rows_affected = wppg_sql_affected_rows( $this->dbh );
			}
			...
			if ( preg_match( '/^\s*(insert|replace)\s/i', $query ) ) {
				if ( $this->use_mysqli ) {
					...
					$this->insert_id = wppg_sql_insert_id( $this->dbh );
				}
			}
			...
			if ( $this->use_mysqli && $this->result instanceof mysqli_result ) {
				...
				while ( $row = wppg_sql_fetch_object( $this->result ) ) {
					$this->last_result[$num_rows] = $row;
					$num_rows++;
				}
			}
	...
	/**
	 * Internal function to perform the wppg_sql_query() call.
	 *
	 * @since 3.9.0
	 *
	 * @access private
	...
	private function _do_query( $query ) {
		...
		if ( ! empty( $this->dbh ) && $this->use_mysqli ) {
			...
			$this->result = wppg_sql_query( $query, $this->dbh );
		}
		$this->num_queries++;
				...
				if ( ! empty( $value['db'] ) ) {
					...
					if ( $this->charset ) {
						...
						if ( $this->use_mysqli ) {
							...
							$connection_charset = wppg_sql_client_encoding();
						}
					}
	...
	protected function load_col_info() {
		...
		if ( $this->use_mysqli ) {
			...
			$num_fields = wppg_sql_num_fields( $this->result );
			for ( $i = 0; $i < $num_fields; $i++ ) {
				$this->col_info[ $i ] = wppg_sql_fetch_field( $this->result, $i );
			}
		}
	}
	 ...
	 *
	 * @global string $wp_version
	 * @global string $required_wppg_sql_version
	 *
	 * @return WP_Error|void
	 */
	public function check_database_version() {
		global $wp_version, $required_wppg_sql_version;
		// Make sure the server has the required MySQL version
		if ( version_compare($this->db_version(), $required_wppg_sql_version, '<') )
			return new WP_Error('database_version', sprintf( __( '<strong>ERROR</strong>: WordPress %1$s requires MySQL %2$s or higher' ), $wp_version, $required_wppg_sql_version ));
	}
		...
		switch ( strtolower( $db_cap ) ) {
			...
			case 'utf8mb4' :      // @since 4.1.0
				...
				if ( $this->use_mysqli ) {
					...
					$client_version = wppg_sql_get_client_info();
				}
	...
	public function db_version() {
		if ( $this->use_mysqli ) {
			...
			$server_info = wppg_sql_get_server_info( $this->dbh );
		}
